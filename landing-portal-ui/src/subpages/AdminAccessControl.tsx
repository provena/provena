...
const StatusSelectorForm = observer((props: StatusSelectorFormProps) => {
  ...
  return (
    <FormControl variant="standard" size="small" disabled={props.disabled}>
      <TextField
        onChange={(event) => {
          const status: RequestStatus = event.target.value as RequestStatus;
...
const UpdateStatusButtonDialog = observer((props: UpdateStatusProps) => {
  ...
  const [desiredStatus, setDesiredStatus] = React.useState(props.currentStatus);
      ...
      <Dialog open={open} onClose={handleCancel}>
        {busy ? (
          ...
          <React.Fragment>
            ...
            <DialogContent>
              <DialogContentText>
                Something went wrong when modifying the record! {failureReason}
              </DialogContentText>
            </DialogContent>
            <DialogActions>
          ...
          <React.Fragment>
            ...
            <DialogContent>
              <DialogContentText>
                Please confirm the new status of this record. <br />
              ...
              <FormGroup>
                <FormControl variant="outlined" size="small">
                  <FormControlLabel
                    ...
                    label={"Send email alert to " + props.email + "?"}
                  ></FormControlLabel>
                </FormControl>
                <StatusSelectorForm
                  handleStatusChoice={(status) => setDesiredStatus(status)}
                  disabled={false}
      ...
      <Dialog open={open} onClose={handleCancel}>
        {failureState ? (
          <React.Fragment>
            ...
            <DialogContent>
              <DialogContentText>
                Something went wrong when deleting the record! {failureReason}
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <Button onClick={handleFailureCancel}>Cancel</Button>
            </DialogActions>
          </React.Fragment>
        ) : (
          <React.Fragment>
  ...
  <Box marginTop={2} marginBottom={2}>
    ...
    <Table size={dense ? "small" : "medium"}>
      {!dense && (
        <TableHead>
          <TableRow key="header">
            <TableCell key="Role display name">Role display name</TableCell>
            <TableCell key="Role name">Role name</TableCell>
            <TableCell key="Access granted">Access granted</TableCell>
          </TableRow>
        </TableHead>
      )}
      <TableBody>
        {roleComponent.component_roles.map((accessRole) => (
          <TableRow key={accessRole.role_name}>
            ...
            <TableCell key={accessRole.role_name + accessRole.access_granted}>
...
const AddNoteDialogButton = observer((props: AddNoteDialogButtonProps) => {
  ...
  const handleAddNote = () => {
    if (noteContent === "") {
      ...
      if (includeTimestamp) {
        timestamp +=
          " (at " + new Date().toLocaleString().replaceAll(",", " -") + ")";
      }
      ...
      <Dialog open={open} onClose={handleCancel}>
        {failureState ? (
          <React.Fragment>
            ...
            <DialogContent>
              <DialogContentText>
                Something went wrong when adding note! {failureReason}
              </DialogContentText>
            </DialogContent>
            <DialogActions>
              <Button onClick={handleFailureCancel}>Cancel</Button>
          ...
          <React.Fragment>
            ...
            <DialogContent>
              <DialogContentText>
                Please specify note you want to add for this request. <br />
              </DialogContentText>
              <FormGroup>
                <FormControl variant="outlined" size="small">
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={handleTimestampIncludeCheckbox}
                      />
                    }
                ...
                <FormControl variant="outlined" size="small">
                  ...
                  <Input id="new note entry" onChange={handleNoteUpdate} />
                </FormControl>
              </FormGroup>
            </DialogContent>
            <DialogActions>
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell key={props.item.username}>{props.item.username}</TableCell>
        <TableCell key={props.item.status}>{props.item.status}</TableCell>
        <TableCell key={props.item.created_timestamp}>
          {new Date(props.item.created_timestamp * 1000).toLocaleString()}{" "}
        </TableCell>
        <TableCell key={props.item.expiry}>
          {Math.ceil(
            (new Date().getTime() -
              new Date(props.item.created_timestamp * 1000).getTime()) /
              (1000 * 60 * 60),
          )}{" "}
          Hours
        </TableCell>
      ...
      <TableRow>
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={8}>
          <Collapse in={expanded} timeout="auto" unmountOnExit>
            ...
            {JSON.parse(props.item.request_diff_contents)["components"].map(
              // @ts-ignore
              (roleComponent) => showRoleInTable(roleComponent, false),
            )}
          </Collapse>
        </TableCell>
      </TableRow>
      ...
      <TableBody>
        {props.items.map((req) => {
          ...
          if (props.statusFilter || props.usernameFilter) {
            if (
              props.statusFilter &&
              !(req.status.valueOf() === props.statusFilter.valueOf())
            ) {
              return null;
            }
            if (
              ...
              !req.username
                .toUpperCase()
                .trim()
                .includes(props.usernameFilter.toUpperCase().trim())
            ) {
              return null;
            }
          }
  ...
  return (
    <React.Fragment>
      <FormGroup>
        ...
        <FormControlLabel
          control={<Checkbox onChange={handleUsernameFilterCheckbox} />}
        ...
        >
          ...
          <Input id="username-query" onChange={handleUsernameFilterUpdate} />
        </FormControl>
      </FormGroup>
    </React.Fragment>
  );
      ...
      <Grid container xs={12} rowGap={1.5}>
        ...
        <Grid xs={6}>
          <RequestTableFilters
            ...
            setUsernameFilterActive={(b) => setUsernameFilterActive(b)}
        ...
        <Grid xs={12}>
          <RequestItemTable
            ...
            statusFilter={statusFilterActive ? statusFilter : undefined}
            usernameFilter={usernameFilterActive ? usernameFilter : undefined}
          ></RequestItemTable>
        </Grid>
      </Grid>
    </div>
