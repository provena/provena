...
const MissingStatusAlertComponent = (
  ...
  const content: JSX.Element = (
    <Stack direction="column" spacing={2}>
      <p>
        To register and manage data, entities and provenance in the system, you
        must to register yourself as a Person in the Registry and link yourself
        to this identity. We have detected that your account is not linked to a
        Person.
      </p>
      {!(props.shouldLink ?? true) && (
        <p>
          However, you are currently a read-only user in the system. You do not
          need to link your account to a Person if you are only intending to
          read data in the information system.
        </p>
      )}
      <p>
...
const SuccessStatusAlertComponent = (
  ...
  const content: JSX.Element = (
    <Stack direction="column" spacing={2}>
      <p>
        Your account is linked to a Person ({props.personId}) in the Registry.
        Click <a href={registryLink}>here</a> to view your linked Person in the
        Registry.
      </p>
    </Stack>
  );
  ...
  return (
    <React.Fragment>
      ...
      <Typography variant="body1">
        Creation and modification of resources in this information system are
        associated with your persistent identity. This persistent identity is
        stored as a Person in the Registry. Before proceeding with the following
        steps, you must register yourself as a Person in the Registry. If you
        already have, you can proceed to the next step. Otherwise,{" "}
        <a href={regDeepLink} target="_blank" rel="noreferrer">
          click here
        </a>{" "}
        to register a Person and return here when you have finished. For more
        help with this process, visit{" "}
...
const SearchSelectPersonComponent = (
  ...
  return (
    <React.Fragment>
      <Typography variant="h6">Search for yourself in the Registry</Typography>
      <Typography variant="body1">
        In the step above, you registered yourself as a Person in the Registry.
        Use the search tool below to select the registered Person.
      </Typography>
    ...
    validationAlertConfig = {
      ...
      body: (
        <p>
          ...
          administrator. The page will refresh shortly...
        </p>
      ),
    };
  }
      ...
      body: (
        <Stack direction="column" spacing={2}>
          ...
          >
            ...
            <Button variant="outlined" color="warning" onClick={props.onClear}>
              Clear selection
            </Button>
            {
              // Submit
  ...
  const readAccessDeniedAlert = (
    <React.Fragment>
      ...
      <Alert variant="outlined" severity="warning">
        ...
        To access and search for resources in the Registry, you must have
        Registry Read permissions. We have detected that you do not have
        sufficient permissions. For help in requesting additional system access,
        please visit our{" "}
  ...
  return (
    <Stack direction="column" spacing={2}>
      {link.loading ? (
        ...
        <Alert variant="outlined" severity="error">
          <AlertTitle>Could not retrieve link status</AlertTitle>An error
          occurred while determining the link status. Error message:{" "}
          {link.errorMessage ?? "Unknown"}. Try refreshing the page. If the
          error persists, contact an adminstrator.
        </Alert>
      ) : (
        <React.Fragment>
          {statusSection}
          {showInstructions && <RegisterPersonInstructionsComponent />}
