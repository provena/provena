...
export class AccessStore {
  ...
  generateAccessReport() {
    requests.get(AUTH_API_ENDPOINTS.GENERATE_ACCESS_REPORT).then((res) => {
      ...
      if (returnedReport.status.success) {
        ...
        this.proposedAccessReport = copyAccessReport(returnedReport.report);
        this.currentAccessReport = copyAccessReport(returnedReport.report);
      }
  ...
  requestRolesUpdate() {
    ...
    requests
      .post(AUTH_API_ENDPOINTS.REQUEST_ACCESS_CHANGE, this.proposedAccessReport)
      .then((response) => {
        let res = response as AccessRequestResponse;
        this.accessRolesUpdating = false;
        if (res.status.success) {
          this.accessRolesRequestingDialog = true;
          this.accessRequestResponseMessage = this.successfulResponseMessage;
