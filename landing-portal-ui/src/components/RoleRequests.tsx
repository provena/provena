      ...
      <TableBody>
        {accessRequestItems.map((req) => (
          <TableRow key={req.request_id}>
            <TableCell key={req.request_id}>{req.request_id}</TableCell>
            ...
            <TableCell key={req.created_timestamp}>
              {new Date(req.created_timestamp * 1000).toLocaleString()}{" "}
            </TableCell>
            <TableCell key={req.expiry}>
              {Math.ceil(
                (new Date().getTime() -
                  new Date(req.created_timestamp * 1000).getTime()) /
                  (1000 * 60 * 60),
              )}{" "}
              Hours
            </TableCell>
            <TableCell>
              {JSON.parse(req.request_diff_contents)["components"].map(
                // @ts-ignore
                (roleComponent) => showRoleInTable(roleComponent, true, true),
              )}
            </TableCell>
          </TableRow>
        ))}
        ...
        {dense ? (
          <TableHead>
            <TableRow key="header">
              ...
              <TableCell key="Access level">Access level</TableCell>
            ...
            if (show) {
              ...
              return (
                <TableRow key={accessRole.role_name}>
                  ...
                  <TableCell key={accessRole.role_name + accessRole.role_level}>
                  ...
                  <TableCell
                    key={accessRole.role_name + accessRole.access_granted}
                  >
                    {dense ? (
                      ...
                      <Switch
                        checked={accessRole.access_granted}
                        onChange={() =>
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={() => props.setIncludeAdmin(!props.includeAdmin)}
              />
            }
...
export const RoleRequests = observer((props: RoleRequestsProps) => {
  ...
  return (
    <React.Fragment>
      ...
      Use the toggle slider in the <i>on</i> position to request access for a
      role. Once you have made a change, use the <i>Request Roles Update</i>{" "}
      button. For more information, visit{" "}
      <a
        href={
        ...
        >
          ...
          >
            <Typography variant="body1" align="left">
              Thank you for making an access request. We will apply these
              changes as soon as possible. In the meantime, you might like to
              return to the home page. You can do so using the button below.
            </Typography>
          </Grid>
          <Grid>
      ...
      >
        ...
        <DialogContent>
          {accessStore.accessRequestResponseMessage ?? "Message loading..."}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => accessStore.closeDialog()}>Confirm</Button>
        </DialogActions>
      </Dialog>
    </React.Fragment>
  );
