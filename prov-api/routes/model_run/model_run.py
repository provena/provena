...
from helpers.entity_validators import (
    RequestStyle,
    validate_model_run_study_linking,
    UserCipherProxy,
)
...
from helpers.job_api_helpers import (
    submit_model_run_lodge_job,
    submit_batch_lodge_job,
    submit_model_run_lodge_only_job,
    submit_model_run_update_job,
)
from ProvenaInterfaces.AsyncJobModels import (
    ProvLodgeModelRunPayload,
    ProvLodgeBatchSubmitPayload,
    ProvLodgeModelRunLodgeOnlyPayload,
    ProvLodgeUpdatePayload,
)
...
@router.post(
    "/register",
    response_model=RegisterModelRunResponse,
    operation_id="register_model_run",
)
async def register_model_run_complete(
    ...
    request_style = RequestStyle(service_account=None, user_direct=roles.user)
    ...
    try:
        ...
        session_id = await submit_model_run_lodge_job(
            ...
            payload=ProvLodgeModelRunPayload(
                record=record, revalidate=False, user_info=user_cipher
            ),
            config=config,
        )
    ...
    return RegisterModelRunResponse(
        status=Status(
            success=True,
            details=f"Job dispatched, monitor session ID using the job API to see progress.",
        ),
...
@router.post(
    "/register_sync",
    response_model=SyncRegisterModelRunResponse,
    operation_id="register_model_run_sync",
...
@router.post(
    f"{EDIT_ROOT_PREFIX}/link_to_study",
    response_model=AddStudyLinkResponse,
    operation_id="add_study_link",
)
async def link_to_study(
    ...
    request_style = RequestStyle(user_direct=roles.user, service_account=None)
    ...
    return AddStudyLinkResponse(
        status=Status(
            success=True,
            details=f"Succesfully linked model run and study together in the registry and lodged a job for the provenance graph update with given session ID.",
        ),
        model_run_id=model_run_id,
        study_id=study_id,
        session_id=session_id,
...
@router.post(
    "/update",
    response_model=PostUpdateModelRunResponse,
    operation_id="update_model_run",
