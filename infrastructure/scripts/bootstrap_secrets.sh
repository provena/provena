...
client_ids=("data-store-api" "service_account_oidc_aws" "registry-api" "prov-api" "prov-job-dispatcher" "auth-api")
secret_names=("${stage}-data-store-api-service-account" "${stage}-data-store-api-oidc-service-account" "${stage}-registry-api-service-account" "${stage}-prov-api-service-account" "${stage}-prov-job-dispatcher-service-account" "${stage}-auth-api-service-account")
descriptions=("Provides client id/client secret and grant type for the data store api keycloak client. Used by data store API automatically. Do not share." "Provides client id/client secret and grant type for the data store api OIDC keycloak client. Used by data store API automatically. Do not share." "Service account for the registry API enabling access to the handle service on behalf of users. Keep secret, do not share or use. Deployment only." "Service account for the prov API enabling access to the registry API on behalf of users. Keep secret, do not share or use. Deployment only." "Service account for the prov job dispatcher. lodges on behalf of the user. Do not share or use. Deployment only." "Auth API service account. Used to allow the Auth API to launch email jobs and perform other tasks")
cdk_variable_names=("data_store_api_service_account_arn" "data_store_oidc_service_account_arn" "registry_api_service_account_arn" "prov_api_service_account_arn" "prov_dispatcher_service_account_arn" "auth_api_service_account_arn")
