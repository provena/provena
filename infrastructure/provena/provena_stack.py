...
from provena.component_constructs.jobs.AsyncJobInfra import (
    AsyncJobInfra,
    JobType,
    JobConfig,
)
...
from provena.component_constructs.static_cloudfront_distribution import (
...
from provena.component_constructs.search_streamers import (
    SearchStreamers,
...
class ProvenaStack(Stack):
    def __init__(
        self, scope: Construct, id: str, config: ProvenaConfig, **kwargs: Any
    ) -> None:
        ...
        if config.components.keycloak is not None:
            ...
            keycloak_auth_endpoint_full = (
                config.general.keycloak_endpoints.full_endpoint
            )
            keycloak_auth_endpoint_minimal = (
                config.general.keycloak_endpoints.minimal_endpoint
            )
            # Keycloak issuer is used for OIDC service relationship on STS assume role with web identity call
            # e.g. auth.dev.provena.io
        ...
        # shared - this comes from parameter
            ...
            oidc_service_role_arn = (
                f"arn:aws:iam::{Stack.of(self).account}:role/DEV-oidc_bucket_service"
            )
        ...
        if config.components.identity_service:
            ...
            id_service = IdentityService(
                ...
                feature_number=config.deployment.ticket_number,
        ...
        if config.components.auth_api:
            ...
            self.auth_api_endpoint = CfnOutput(
                self, "auth-api-endpoint-output", value=auth_api.endpoint
            )
        ...
        if config.components.search:
            ...
            open_search_infra = OpenSearchCluster(
                ...
                existing_domain_arn=search_config.existing_cluster_info.domain_arn
                if search_config.existing_cluster_info
                else None,
                existing_domain_endpoint=search_config.existing_cluster_info.domain_endpoint
                if search_config.existing_cluster_info
                else None,
            )
            ...
            self.search_api_endpoint = CfnOutput(
                self, "search-api-endpoint-output", value=search_api.endpoint
            )
        ...
        if config.components.entity_registry:
            ...
            self.registry_api_endpoint = CfnOutput(
                self, "registry-api-endpoint-output", value=registry_api.endpoint
            )
            ...
            self.registry_ui_bucket_name = CfnOutput(
                scope=self, id="registry_ui_bucket_name", value=registry_ui.bucket_name
            )
        ...
        if config.components.data_store:
            ...
            self.data_store_ui_bucket_name = CfnOutput(
                scope=self, id="data_store_ui_bucket_name", value=ds_ui.bucket_name
            )
            ...
            self.data_api_endpoint = CfnOutput(
                self, "data-api-endpoint-output", value=data_api.endpoint
            )
        ...
        if config.components.prov_store:
            ...
            self.prov_api_endpoint = CfnOutput(
                self, "prov-api-endpoint-output", value=prov_api.endpoint
            )
            ...
            self.prov_ui_bucket_name = CfnOutput(
                scope=self, id="prov_ui_bucket_name", value=prov_ui.bucket_name
            )
        ...
        registry_job_environment: Dict[str, str] = {
            k: v
            for k, v in registry_api.registry_api_environment.items()
            if v is not None
        }
        ...
        email_secret = aws_sm.Secret.from_secret_complete_arn(
            scope=self,
            id="email-secret",
            secret_complete_arn=config.general.email_connection_secret_arn,
        )
            ...
            JobConfig(
                ...
                image=ecs.ContainerImage.from_asset(
                    ...
                    build_args={
                        ...
                        "branch_name": config.deployment.git_branch_name,
                        "CACHE_BUSTER": hash_dir_list(
                            async_config.registry_job_extra_hash_dirs
                        ),
                    },
                ),
                visibility_timeout=Duration.minutes(5),
            ...
            JobConfig(
                ...
                image=ecs.ContainerImage.from_asset(
                    ...
                    build_args={
                        ...
                        "branch_name": config.deployment.git_branch_name,
                        "CACHE_BUSTER": hash_dir_list(
                            async_config.email_job_extra_hash_dirs
                        ),
                    },
                ),
                visibility_timeout=Duration.minutes(5),
                ...
                secrets={
                    ...
                    "port": ecs.Secret.from_secrets_manager(
                        secret=email_secret, field="port"
                    ),
                    # smtp_server: str
                    "smtp_server": ecs.Secret.from_secrets_manager(
                        secret=email_secret, field="smtp_server"
                    ),
                    # Username/password
                    # username: str
                    "username": ecs.Secret.from_secrets_manager(
                        secret=email_secret, field="username"
                    ),
                    # password: SecretStr
                    "password": ecs.Secret.from_secrets_manager(
                        secret=email_secret, field="password"
                    ),
                    # Email from address
                    # email_from: str
                    "email_from": ecs.Secret.from_secrets_manager(
                        secret=email_secret, field="email_from"
                    ),
                },
            ),
        ]
        ...
        if config.components.warmer is not None:
            ...
            warmer = LambdaWarmer(
                ...
                endpoints=WarmerEndpoints(
                    ...
                    HANDLE_API_ENDPOINT=id_service.handle_endpoint
                    if id_service
                    else None,
            ...
            self.warmer_api_endpoint = CfnOutput(
                self, "warmer-api-endpoint-output", value=warmer.endpoint
            )

        # Expose the endpoint
        self.job_api_endpoint = CfnOutput(
            self, "job-api-endpoint-output", value=async_infra.job_api_endpoint
        )
        ...
        if backup_config.backups_enabled:
            self.backup_service = BackupService(
                ...
                key_admins=[
                    iam.ArnPrincipal(arn) for arn in (backup_config.key_admins or [])
                ],
            )

            def backup_helper(
                id: str, type: BackupType, resource: backup.BackupResource
            ) -> None:
