...
class AsyncJobInfra(Construct):
    def __init__(
        ...
        **kwargs: Any,
        ...
        oauth_token = direct_import(github_build_token_arn, Stack.of(self).region)
        ...
        acm_cert = aws_cm.Certificate.from_certificate_arn(self, "api-cert", cert_arn)
        ...
        api = api_gw.LambdaRestApi(
            ...
            domain_name=api_gw.DomainNameOptions(
                domain_name=f"{domain}.{allocator.root_domain}", certificate=acm_cert
            ),
            deploy_options=api_gw.StageOptions(
                description="Async Job API Docker Lambda FastAPI API Gateway",
                throttling_burst_limit=api_rate_limiting.throttling_burst_limit
                if api_rate_limiting
                else None,
                throttling_rate_limit=api_rate_limiting.throttling_rate_limit
                if api_rate_limiting
                else None,
            ),
        )
        ...
        global_list_index_name = (
            gsi_field_key + "-" + timestamp_key + INDEX_NAME_POSTFIX
        )
        ...
        status_table.add_global_secondary_index(
            ...
            partition_key=ddb.Attribute(
                name=username_key, type=ddb.AttributeType.STRING
            ),
        ...
        status_table.add_global_secondary_index(
            ...
            partition_key=ddb.Attribute(name=batch_key, type=ddb.AttributeType.STRING),
            sort_key=ddb.Attribute(name=timestamp_key, type=ddb.AttributeType.NUMBER),
        )
        ...
        cluster: ecs.Cluster = ecs.Cluster(
            scope=self, id="cluster", vpc=vpc, enable_fargate_capacity_providers=True
        )
        ...
        connector.add_environment(key="TABLE_NAME", value=status_table.table_name)
        ...
        for job in jobs:
            ...
            job_api.function.add_environment(
                key=job.type + "_TOPIC_ARN", value=topic.topic_arn
            )
            ...
            topic.add_subscription(sns_subs.SqsSubscription(queue=queue))
            # Subscribe the invoker to topic
            topic.add_subscription(sns_subs.LambdaSubscription(fn=invoker))

            # Subscribe the ddb connector to topic

            topic.add_subscription(sns_subs.LambdaSubscription(fn=connector))
            ...
            invoker.add_environment(
                ...
                key=job.type + "_TASK_DEFINITION_ARN",
                value=task_dfn.task_definition_arn,
            )

            # Let the invoker invoke the task definition
            task_dfn.grant_run(invoker)

        invoker.add_environment(key="vpc_id", value=vpc.vpc_id)
        invoker.add_environment(key="idle_timeout", value=str(idle_timeout))
        invoker.add_environment(key="max_task_scaling", value=str(max_task_scaling))
