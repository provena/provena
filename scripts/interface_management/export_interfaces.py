...
def delete_files_in_folder(folder_path: str) -> None:
    ...
    if os.path.isdir(folder_path):
        ...
        for filename in os.listdir(folder_path):
            ...
            # If it's a file, delete it. If it's a directory, skip it.
...
def ui_to_path(ui: UI) -> str:
    ui_target_path = UI_TO_PATH[ui]
    return os.path.join(UI_RELATIVE_BASE, ui_target_path, POSTFIX)
...
def construct_target_path(source: SOURCE, ui: UI) -> ExportDefinition:
    ...
    return ExportDefinition(
        python_module_location=os.path.join(
            SHARED_INTERFACES_LOCATION, source_to_python_source_name(source)),
        export_file_location=os.path.join(ui_to_path(ui), exported_name)
    )
...
def construct_copy_from_to_paths(source: SOURCE, ui: UI, tmpdirname: str) -> Tuple[str, str]:
    ...
    export_location = os.path.join(
        UI_RELATIVE_BASE, ui_target_path, POSTFIX, exported_name)
...
with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    for source in ALL_SOURCES:
        ...
        # Do this once
        print(
            f"Exporting from {export_def.python_module_location} ---> {export_def.export_file_location}...")
        generate_typescript_defs(
            module=export_def.python_module_location,
            output=export_def.export_file_location
        )

    # then clean up all export locations before exporting
    for ui in UI:
        target_folder = ui_to_path(ui)
        print(f"Cleaning up the folder: {target_folder}...")
    ...
    for ui, source_list in TARGETS.items():
        for source in source_list:
            copy_list.append(
                construct_copy_from_to_paths(
                    source=source, ui=ui, tmpdirname=tmpdirname)
            )
