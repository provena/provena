#!/bin/bash
# Script for deploying the pipeline infrastructure
...
usage() {
    ...
    echo
    echo "Arguments:"
    echo "  namespace/stage/config     The namespace/stage/config for the pipeline stack"
    echo
    echo "Options:"
    ...
    echo
    echo "Example:"
    echo "  $0 org/dev/dev --target https://github.com/org/config-repo.git"
    exit 1
}
...
python3.10 app_run.py $config_name pipeline "deploy --require-approval never"
