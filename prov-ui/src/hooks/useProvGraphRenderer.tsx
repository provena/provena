...
import { DOCUMENTATION_BASE_URL, getSwatchForSubtype } from "react-libs";
    ...
    return (
        <Stack direction="row" justifyContent="space-between">
            ...
            >
                ...
                <IconButton onClick={props.resetGraph} title="Restart Graph">
...
export const useProvGraphRenderer = (
    ...
    */
    ...
    // Maintain height/width reference and set observer for layout changes to width
    useLayoutEffect(() => {
        // Only attempt to pull height/width if current
        if (svgDivRef.current) {
            setHeight(svgDivRef.current.clientHeight);
            setWidth(svgDivRef.current.clientWidth);
        }

        // Setup a observer for width - runs on div size change
        if (svgDivRef.current) {
            ...
            resizeObserver.observe(svgDivRef.current);
            // Clean up
            return () => {
        ...
        // Run this use effect when the div ref changes
        svgDivRef?.current,
    ]);
