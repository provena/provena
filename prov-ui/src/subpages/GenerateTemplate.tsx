...
const WorkflowTemplateList = observer((props: WorkflowTemplateListProps) => {
  ...
  const searchSection = (
    <FormControl>
      <Typography mb={"15px"} variant={"subtitle1"}>
        Enter a search query to search for a registered workflow template.
        Select the desired template from the results.
      </Typography>
      <Grid container>
        ...
        >
          <IconButton>
            {searchResults.loading ? <CircularProgress /> : <SearchIcon />}
          </IconButton>
        </Grid>
      </Grid>
    </FormControl>
  ...
  if (searchResults.error) {
    ...
    const columns: GridColDef[] = [
      ...
      { field: "versionNumber", headerName: "Version" },
    ...
    const rows: GridRowsProp = items
      ...
      .map((modelWorkflowTemplate) => {
        return {
          ...
          versionNumber: modelWorkflowTemplate.versioning_info?.version ?? "",
  ...
  return (
    <Grid item xs={12}>
      {isFetching ? (
        ...
        <React.Fragment>
          An error occurred while requesting a CSV template, error message:{" "}
          {error}.
        </React.Fragment>
