...
import { SortType } from "../provena-interfaces/RegistryAPI";
...
const DatasetList = observer(() => {
  ...
  const [sortSelection, setSortSelection] = useState<SortType>(defaultSortType);
        ...
        <DialogContent>
          <DialogContent>
            <p>
              ...
              <br />
              Press <i>Dismiss</i> to attempt loading from the data store
        ...
        <Grid container>
          <Grid xs={3}>
            ...
            <OrganisationFilterDisplay
              ...
              selectedOrganisations={filteredByOrg.selectedOrganisations}
              addOrganisation={filteredByOrg.selectOrganisation}
              removeOrganisation={filteredByOrg.deselectOrganisation}
            />
          </Grid>
          <Grid xs={9}>
              ...
              {splicedDatasets.length > 0 ? (
                <>
                  {splicedDatasets.map((ele) => (
                    <DatasetSummary key={ele.id} dataset={ele}></DatasetSummary>
                  ))}
                  {!usingSearch && paginatedDatasetList.moreAvailable && (
                    <Grid
                      container
        ...
        <Grid container>
          <Grid xs={12}>
            <Grid className={classes.gridMargin}>
              <h3>Unauthorised</h3>
              <p>
                You have insufficient permissions to use the data store
                registry. You can manage your roles at{" "}
