...
export const ApprovalSubmission = () => {
  ...
  // For Select
  const [selectInvalidInput, setSelectInvalidInput] = useState<boolean>(false);
  // For Checkbox
    ...
    return (
      <Stack direction="column" className={classes.root} spacing={2}>
        <Alert variant="outlined" severity="error">
          <AlertTitle>
            Error: An error occurred when fetching the list of approvers.
          </AlertTitle>
      ...
      {fetchLoading ? (
        ...
        <form onSubmit={handleSubmit}>
          <Stack direction="column" spacing={4}>
              ...
              <Stack>
                ...
                >
                  {approversList.complete &&
                    ...
                    approversItemList.map(
                      (approverLoadedItem: ItemDataset | undefined) => (
                        <MenuItem
                          key={approverLoadedItem?.id}
                          value={approverLoadedItem?.id}
                        >
                          <UserDisplayName
                            ownerName={approverLoadedItem?.owner_username}
                          />
                          {" - "}
                ...
                {selectInvalidInput && (
                  <FormHelperText>
                    ...
                    >
                      Please select an approver from the list to process the
                      approval.
                    </Typography>
                  </FormHelperText>
                )}
              ...
              {checkboxInvalidInput && (
                <FormHelperText>
                  <Typography
                    ...
                    color={theme.palette.error.main}
                  >
                    You must confirm this checkbox to process the approval
                    request.
                  </Typography>
                </FormHelperText>
              )}
            ...
            {/* Action buttons */}
            <Grid container item xs={12} className={classes.actionButtons}>
              <Grid item>
                <Button
                  variant="contained"
                  type="button"
            ...
            >
              <Stack direction="column" className={classes.fullWidth} p={4}>
                <Alert variant="outlined" severity="success">
                  <AlertTitle>
                    Success: Your approval request has been submitted
                    successfully.
                  </AlertTitle>
          ...
          {isError && (
            ...
            >
              <Alert variant="outlined" severity="error">
                <AlertTitle>
                  Error: An error occurred when submitting the approval request.
                </AlertTitle>
                {approvalSubmit.error ?? "Error message is not provided."}
              </Alert>
            </Stack>
          )}
        </form>
