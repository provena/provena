...
const UpdateMetadata = observer((props: UpdateMetadataProps) => {
  ...
  const dialogFragment = (
    <React.Fragment>
      ...
      <Dialog open={updateStore.error}>
        ...
        <DialogContent>
          <DialogContent>
            <p>{updateStore.errorMessage ?? "No error message provided."}</p>
          </DialogContent>
        </DialogContent>
        <DialogActions>
  ...
  if (updateStore.schemaReady) {
      ...
      <div className={classes.root}>
        <Stack direction="column" spacing={2}>
          ...
          <Form
            ...
            onError={(errors) => {
              console.log("Errors");
              console.log(JSON.stringify(updateStore.currentValue));
            }}
            onSubmit={({ formData }) => {
              ...
              switch (props.mode) {
                case "edit": {
                  reason !== undefined &&
                    updateStore.updateMetadataAction(formData, history, reason);
                  break;
                }
                case "clone": {
                  updateStore.registerDatasetAction(formData, history);
                  break;
                }
              }
            }}
          ...
          >
            <Stack direction="column" spacing={2}>
              ...
              {!submitEnabled && submitDisabledReason !== undefined && (
                <Grid item xs={12}>
    ...
    return (
      <div className={classes.root}>
        Schema form retrieval failure - try refreshing once you have resolved
        the errors.
        {dialogFragment}
      </div>
    );
