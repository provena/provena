...
} from "react-libs/provena-interfaces/DataStoreAPI";
import { DatasetMetadata } from "react-libs/provena-interfaces/RegistryModels";
import { HTTPValidationError } from "../provena-interfaces/APIResponses";
...
export class RegisterOrModifyStore {
      ...
      requests
        ...
        .catch((e) => {
          ...
          if (e.response) {
            ...
            errorMessage = "Failed to load schema! Unknown error occurred.";
          }
          reject(errorMessage);
        });
    });
  ...
  prepareDefaultValueExisting(handle_id: string) {
    ...
    requests
      ...
      .then((res) => {
        const response: RegistryFetchResponse = res as RegistryFetchResponse;
  ...
  registerNewDataset(inputData: DatasetMetadata) {
    return new Promise((resolve, reject) => {
      ...
      requests
        .post(DATA_STORE_API_ENDPOINTS.MINT_DATASET, inputData, {}, {}, true)
        .then((response) => {
        ...
        .catch((e) => {
          ...
          if (e.response) {
            if (e.response.status == 422) {
              ...
              errorMessage =
                "Validation errors: " + displayValidationError(valError);
            } else {
              errorMessage = `Unexpected error when performing dataset registration, status code: ${
                e.response.status
              }, and reason: ${e.response.data.detail ?? "Unknown"}`;
            }
          } else if (e.message) {
  ...
  updateMetadata(inputData: DatasetMetadata, reason: string) {
    return new Promise((resolve, reject) => {
      ...
      requests
        ...
        .then((response) => {
          let res: UpdateMetadataResponse = response as UpdateMetadataResponse;
          if (res.status.success) {
            // Successful - notify
            resolve(true);
          } else {
  ...
  validateMetadata(formData: JSON) {
    return new Promise((resolve, reject) => {
      ...
      requests
        .post(DATA_STORE_API_ENDPOINTS.VALIDATE_METADATA, inputData, {}, true)
        .then((response) => {
          ...
          if (res.success == true) {
            ...
            const errorMessage = `Failed to validate updated metadata, error details: ${
              res.details ?? "No details provided. Contact administrator."
            }`;
            reject(errorMessage);
          }
        })
        .catch((e) => {
          ...
          if (response) {
            if (response.status == 422) {
              ...
              errorMessage =
                "Validation error: " + displayValidationError(valError);
              console.log(errorMessage);
            } else {
              errorMessage = `Unexpected error when performing metadata validation, status code: ${
                response.status
              }, and reason: ${response.data.detail ?? "Unknown"}`;
            }
          } else if (e.message) {
