...
const parseFormData = (formData: FormData): ParsedFormData => {
  ...
  const output = {
    reposited: formData["reposited"] ?? (uriOrDesc ? false : DEFAULT_REPOSITED),
...
const AccessInfoOverride = (props: OverrideProps) => {
  ...
  return (
    <Stack spacing={2.5}>
        ...
        <Grid item>
          <FormGroup>
            <FormControlLabel
              control={
                <Checkbox
                  checked={parsedFormData.reposited ?? DEFAULT_REPOSITED}
                  onChange={(e) => {
                    const current =
                      parsedFormData.reposited ?? DEFAULT_REPOSITED;
      ...
      >
        <AlertTitle>
          {repositedDisplay ? "Stored in the data store" : "Stored elsewhere"}
        </AlertTitle>
        {repositedDisplay
