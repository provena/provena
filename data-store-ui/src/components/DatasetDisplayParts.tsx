...
import {
  ItemOrganisation,
  ItemPerson,
} from "react-libs/provena-interfaces/RegistryModels";
  ...
  return (
    <div>
      ...
      {loadingAuthorLink ? (
        ...
        <p>The user does not have a linked Person ID. No details available.</p>
      ) : loadingAuthorDetails ? (
        <p>Loading author details, please wait ...</p>
      ) : authorDetailsError ? (
        <p>
          The user's details could not be fetched from the registry. Error:{" "}
          {loadedAuthor.errorMessage ?? "Unknown"}
        </p>
      ) : (
        successful &&
        typedAuthorData && (
          <React.Fragment>
            ...
            {typedAuthorData.email ? emailLink(typedAuthorData.email) : ""}
            {!!orcidLink && (
              <>
                <h3>ORCID:</h3>
                {orcidLink}
  ...
  return (
    <div>
      ...
      {loadingOrgDetails ? (
        ...
        <p>
          The Organisation's details could not be fetched from the registry.
          Error: {loadedOrg.errorMessage ?? "Unknown"}
        </p>
      ) : (
        successful &&
        typedOrgData && (
    ...
    <div>
      <div>
        <ButtonGroup variant="outlined">
          ...
          <Button
            color={useWindowsPowershellFormat ? "success" : "secondary"}
            onClick={() => {
              setUseBashFormat(false);
              setUseWindowsCmdFormat(false);
