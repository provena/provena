...
import { GetJobResponse } from "../provena-interfaces/AsyncJobAPI";
...
export const useWorkflowManager = (
    ...
    queries: resolvedSessionInfoList.map((session, index) => {
      return {
        ...
        onSuccess: (data: GetJobResponse) => {
          ...
          const gen = props.workflowDefinition.steps[index].nextStepGenerator;

          if (!gen) {
            console.log("No next step found - must be completed workflow.");
            return;
          }

          // check there is a result payload
          if (!data.job.result) {
            console.error("Successful job didn't include a result payload!");
            ...
            resolvedSessionInfoList[index].workflowError = workflowError;
            resolvedSessionInfoList[index].actionMessage = actionMessage;
  ...
  const resolvedWorkflowData: WorkflowData = workflowQueryData.map((data) => {
    return {
      ...
      isAutoRefetching:
        ["FAILED", "SUCCEEDED"].indexOf(data.data?.job.status ?? "") === -1 &&
        enabledRefetch,
      data: data.data,
