...
import { ItemSubType } from "../provena-interfaces/AuthAPI";
    ...
    linkLineSelection.each(function (d: GraphLink) {
      if (
        ...
        if (to.x && to.y && from.x && from.y) {
          ...
          d.targetX = x2 - Math.cos(angle) * (nodeRadius + arrowWidth);
          d.targetY = y2 - Math.sin(angle) * (nodeRadius + arrowWidth);
        }
      }
    });
...
const refreshSimulation = (
  ...
  simulation = simulation
    ...
    .force(
      "collide",
      d3.forceCollide(nodeRadius + (physicsParams.collision.radiusOffset ?? 0)),
    )
    .force(
      "link",
      d3
...
const mergeGraphDataIntoD3 = (
  ...
  graph.links.forEach((link, index) => {
    ...
    // Already strings
      ...
      if (
        foundLinkList.findIndex((otherLink) => {
          return otherLink.from === link.source && otherLink.to === link.target;
        }) === -1
      ) {
        toDeleteLinkIndexes.push(index);
      }
...
const ControlDrawer = observer((props: ControlDrawerProps) => {
  ...
  const upstreamTooltipsTitle = (
    <div>
      ...
      <a href={provExplorationDoc} target="_blank">
        <strong className={classes.buttonTooltipsEmphasize}>upstream</strong>
      </a>{" "}
      only.
    </div>
  );
  const downstreamTooltipsTitle = (
    <div>
      Explore{" "}
      <a href={provExplorationDoc} target="_blank">
        <strong className={classes.buttonTooltipsEmphasize}>downstream</strong>
      </a>{" "}
      only.
    </div>
  );
      ...
      >
        <Box className={classes.iconGroupBox}>
          <Tooltip title={upstreamTooltipsTitle} arrow>
            ...
            >
              <ArrowCircleUpTwoTone className={classes.buttonDrawerIcons} />
            </IconButton>
          </Tooltip>
          <Tooltip title={downstreamTooltipsTitle} arrow>
            ...
            >
              <ArrowCircleDownTwoTone className={classes.buttonDrawerIcons} />
            </IconButton>
          </Tooltip>
...
export const useProvGraphRenderer = (
  ...
  // State
  const [graphData, setGraphData] = useState<NodeGraph | undefined>(undefined);
      ...
      <div>
        <Stack direction="column">
          ...
          <ControlDrawer
            ...
            setUpstreamExpansion={props.controls.setUpstreamExpansion}
            setDownstreamExpansion={props.controls.setDownstreamExpansion}
