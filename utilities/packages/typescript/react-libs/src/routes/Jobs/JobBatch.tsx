...
import { ProvLodgeModelRunResult } from "../../provena-interfaces/AsyncJobModels";
...
export const BatchDetailedView = (props: BatchJobDetailedViewProps) => {
      ...
      .map((job) => {
        ...
        if (job.status === "SUCCEEDED") {
          if (job.job_sub_type === "MODEL_RUN_PROV_LODGE") {
            const result = job.result as unknown as ProvLodgeModelRunResult;
      ...
      <Stack spacing={2}>
        <Grid container justifyContent="space-between" alignItems="center">
          ...
          {allSuccess ? (
            ...
            <Grid item xs={3}>
              <Typography variant="subtitle1">
                This batch contains jobs which did not finish successfully.
              </Typography>
            </Grid>
          ) : (
            isPending && (
              <React.Fragment>
                <Grid item xs={3}>
                  <Typography variant="subtitle1">
                    This batch is pending completion, see below.
                  </Typography>
                </Grid>
              </React.Fragment>
            )
  ...
  if (isError) {
    return (
      <p>
        An error occurred while fetching the batch job status. Error: {error}
      </p>
    );
  }
...
export const JobBatchComponent = (props: JobBatchComponentProps) => {
  ...
  if (!batchId) {
    return rootDisplay(
      <p>
        Invalid path, no {BATCH_ID_QSTRING} query string provided. Return{" "}
        <a href="/">home</a>.
      </p>,
    );
  }
