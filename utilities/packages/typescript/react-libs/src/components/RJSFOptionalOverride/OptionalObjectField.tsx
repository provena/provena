...
> extends Component<FieldProps<T, S, F>, ObjectFieldState> {
    ...
    state = {
        ...
        // Include optional defaults to the current state
        includeOptional: !!this.props.formData,
    };
    ...
    handleAddClick = (schema: S) => () => {
        ...
        set(
            newFormData as GenericObjectType,
            newKey,
    ...
    render() {
                    ...
                    <p className="config-error" style={{ color: "red" }}>
                        <Markdown>
                            {translateString(
                                ...
                                [name || "root", (err as Error).message]
                            )}
                        </Markdown>
                    </p>
        ...
        const showOptionalInclude = !required && !uiSchemaDisableOptionalSwitch;

        const disableChildren =
            showOptionalInclude && !this.state.includeOptional;

        // What title to use?
        const appropriateTitle = uiOptions.label === false ? "" : title;

        const templateProps = {
            // getDisplayLabel() always returns false for object types, so just check the `uiOptions.label`
            title: appropriateTitle,
            description: uiOptions.label === false ? undefined : description,
            properties: disableChildren
                ? // If we have not included then don't render the children
                  []
                : orderedProperties.map((name) => {
                      ...
                      const fieldUiSchema = addedByAdditionalProperties
                          ...
                          : uiSchema[name];
                      const hidden =
                          getUiOptions<T, S, F>(fieldUiSchema).widget ===
                          "hidden";
                      const fieldIdSchema: IdSchema<T> = get(
                          idSchema,
                          [name],
                          {}
                      );
                          ...
                          content: (
                              <SchemaField
                                  ...
                                  wasPropertyKeyModified={
                                      this.state.wasPropertyKeyModified
                                  }
            ...
            formContext,
            registry,
            optionalSwitchEnabled: showOptionalInclude,
            optionalSwitchValue: this.state.includeOptional,
            toggleOptionalSwitchValue: () => {
                this.setState((prevState) => {
                    if (prevState.includeOptional) {
                        console.log("Setting form data to undefined");
                        // We are now swapping to excluding optional so we to undefined
                        this.props.onChange(undefined);
                    }
                    return {
                        ...prevState,
                        includeOptional: !prevState.includeOptional,
                    };
                });
            },
        };
