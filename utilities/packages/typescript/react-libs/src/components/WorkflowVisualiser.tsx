...
import { GetJobResponse, JobStatus } from "../provena-interfaces/AsyncJobAPI";
...
const RenderJobEntryComponent = (props: RenderJobEntryComponentProps) => {
  ...
  const jobErrorAlert: JSX.Element = (
    <Alert variant="outlined" severity="error" className={classes.fullWidth}>
  ...
  if (sessionId !== undefined) {
    ...
    actionSuggestion = props.otherSessionInfo[props.jobIndex]?.actionMessage;
  }
      ...
      >
        {job.error ? (
          ...
          <Stack direction="column" spacing={1.5}>
            ...
            <Stack direction="row" className={classes.jobCardTopRow}>
            ...
            {errorMessage ? (
              ...
              >
                ...
                {errorMessage && (
                  <Typography variant="body1">
                    Action suggested :{" "}
                    {actionSuggestion ?? "No action provided"}
                  </Typography>
                )}
              </Alert>
            ) : (
          ...
          <Stack direction="column" spacing={2}>
            ...
            {workflow.complete && workflow.error && (
              <Alert variant="outlined" severity="error">
                <AlertTitle>Error: Some jobs could not be fetched</AlertTitle>
                Error: Some jobs could not be fetched during the workflow
                generation process.
              </Alert>
            )}
            {!!workflow.workflowData && (
              <Stack direction={props.direction} className={classes.jobWrapper}>
                {workflow.workflowData.map((data, index) => {
                  return (
                    <RenderJobEntryComponent
                      isAutoRefetching={data.isAutoRefetching}
                      job={data}
                      jobIndex={index}
                      workflowStep={props.workflowDefinition.steps[index]}
                      otherSessionInfo={workflow.sessionInfo}
                    />
                  );
                })}
              </Stack>
