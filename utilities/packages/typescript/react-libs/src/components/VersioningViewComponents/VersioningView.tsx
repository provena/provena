...
import { ItemBase } from "../../provena-interfaces/RegistryAPI";
  ...
  if (listLength > 2) {
    ...
    const topExpandVersions: Array<VersioningItem | undefined> = list
      ...
      .map((item: LoadedEntity<ItemBase>) => {
        return !!item.data ? itemBaseToVersioningItem(item.data) : undefined;
      });
...
export const VersioningView = (props: VersioningViewProps) => {
  ...
  if (item.versioning_info === undefined) {
    return (
      <Alert variant="outlined" severity="error" className={classes.fullWidth}>
        <AlertTitle>
          Error: Current entity's versioning information could not be found
        </AlertTitle>
        Error occurred when getting current entity's versioning information
      </Alert>
    );
  }
  ...
  const currentVersion: VersioningItem = {
    ...
    ownerUsername: item.owner_username,
    version: !!item.versioning_info ? item.versioning_info.version : undefined,
    createdTimestamp: item.created_timestamp,
  };
  ...
  const versioningListView = versioningListErrorOccurred ? (
    <React.Fragment>
      <Alert variant="outlined" severity="error" className={classes.fullWidth}>
        <AlertTitle>Error: Versioning list could not be retrieved</AlertTitle>
    ...
    <Stack direction="column" spacing={1}>
      ...
      <Grid container xs={12} className={classes.root}>
        ...
        <Grid container item xs={12} className={classes.toolBar}>
          ...
          {/* Action buttons view */}
          <Grid container item xs={8} className={classes.rhActionButtons}>
            {showCreateNewVersion && (
              <Grid item className={classes.actionButton}>
                {isHeadVersion ? (
                  ...
                  <Tooltip title={createNewVersionTooltipText}>
                    <Box>
