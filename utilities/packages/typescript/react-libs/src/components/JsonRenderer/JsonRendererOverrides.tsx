...
export const jobBatchLinkOverride: RendererOverride<string> = (
  ...
  const rhContent = (
    <Typography className={classes.valueText}>
      <Link to={`${JOB_BATCH_ROUTE}?${BATCH_ID_QSTRING}=${val}`}>{val}</Link>
    </Typography>
  );
...
export const linkedUsernameOverride: RendererOverride<string> = (
  ...
  const boxedContent = objectRendererBoxer({
    interior: <LinkedUsernameToJsonDetails username={val} jsonProps={props} />,
    style: props.style,
  });
...
export const ISO8601DurationOverride: RendererOverride<string> = (
  ...
  const lhContent = (
    <Typography className={classes.boldText}>
      <>
        {prettifyRecordName(name)} (
        <a href={ISO8601_DURATION_LINK} target="_blank" rel="noreferrer">
          ISO8601 Duration
        </a>
        )
      </>
        ...
        rhContent: objectRendererBoxer({
          ...
          interior: (
            <Stack direction="column" spacing={ROW_GAP}>
              <SubtypeHeaderComponent item={item! as ItemBase} compact={true} />
              {rhRenderedObject}
            </Stack>
          ),
        }),
...
/*
The below set of renderers apply a render override function under the following conditions
1. The field name matches the specified field name which is the map key
2. The context of the field is not one of the specified exclusions, if any.

Note that these are separated by type so that the system is fully typed.
*/
export const STRING_RENDER_OVERRIDES: AllFieldRendererMap<string> = new Map([
  // IDs/handles that shouldn't resolve
  ["id", { renderer: handleOverride }],

  // Show the owner username as a special component
  ["owner_username", { renderer: linkedUsernameOverride }],
  ...
  [
    "linked_person_id",
    {
      renderer: subtypedIdResolverOverrideGenerator({
        subtype: "PERSON",
  ...
  [
    "to_version_id",
    {
      renderer: untypedIdResolverOverrideGenerator({
        fieldnameOverride: "To Version",
      }),
    },
  ],
  [
  ...
  [
    "data_custodian_id",
    {
      renderer: subtypedIdResolverOverrideGenerator({
        subtype: "PERSON",
        fieldnameOverride: "Dataset Custodian",
      }),
    },
  ],
  [
  ...
  ["orcid", { renderer: orcidOverride }],
  ["ror", { renderer: rorOverride }],
  ["email", { renderer: emailOverride }],
  // the S3 URI field name override doesn't work - okay for now
  ["s3_uri", { renderer: copyableOverride }],
  ["bucket_name", { renderer: copyableOverride }],
  ["path", { renderer: copyableOverride }],

  // Job overrides
  ["session_id", { renderer: jobSessionLinkOverride }],
  ["lodge_session_id", { renderer: jobSessionLinkOverride }],
  ["batch_id", { renderer: jobBatchLinkOverride }],

  // Workflow Links
  ["version_activity_workflow_id", { renderer: jobSessionLinkOverride }],

  // Versioning Info
  ["previous_version", { renderer: handleOverride }],
  ["next_version", { renderer: handleOverride }],
  ...
  ["release_status", { renderer: releaseStatusOverride }],
  ["action", { renderer: releaseStatusOverride }],
...
export const ARRAY_RENDER_OVERRIDES: AllFieldRendererMap<object> = new Map([
  ["release_history", { renderer: releaseHistoryOverride }],
]);
...
export const NUMBER_RENDER_OVERRIDES: AllFieldRendererMap<number> = new Map([
  ["updated_timestamp", { renderer: timestampOverride }],
  ["created_timestamp", { renderer: timestampOverride }],
  ["start_time", { renderer: timestampOverride }],
  ["end_time", { renderer: timestampOverride }],
  // Release
  ["release_timestamp", { renderer: timestampOverride }],
  ["timestamp", { renderer: timestampOverride }],
]);

// Maps field name + Context -> Override
// Ensure that the SUBTYPE is included as first context argument
...
export function checkFieldRenderWithPossibleExclusion<T>(args: {
  renderMap: AllFieldRendererMap<T>;
  fieldName: string;
  context: string;
}): RendererOverride<T> | undefined {
  /*
    Applies logic to check for field name based render overrides given a field
    name, context and the relevant renderer map. Will check if a) matching path
    b) exclusion applies.
  */
  ...
  // If not matching path then just return no override
  if (!possibleOverride) {
    return undefined;
  }

  // renderer is defined
  const renderer = possibleOverride.renderer;

  // Otherwise check if list of exclusions
  const exclusionList = possibleOverride.exclusions;

  if (!exclusionList) {
    return renderer;
  }

  // There is a renderer and a list of exclusions - check list against context
  for (const exclusion of exclusionList) {
    console.log(`Checking ${exclusion} against ${args.context} context.`);
    if (exclusion === args.context) {
      // Exclusion applied
      return undefined;
    }
  }

  // Otherwise return renderer with respect to exclusions
  return renderer;
}
