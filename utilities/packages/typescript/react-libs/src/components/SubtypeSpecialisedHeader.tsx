        ...
        <Grid item>
          <Grid container spacing={2}>
            <Grid item>
              ...
              >
                {mapSubTypeToPrettyName(props.item.item_subtype)}
                {(props.isUser ?? false) && " (You)"}
                {!!props.item.versioning_info?.version &&
                  subtypeHasVersioning(props.item.item_subtype) &&
                  ` (V${props.item.versioning_info?.version})`}
              </Typography>
            </Grid>
            {(props.locked ?? false) && (
              <Grid item>
                {props.lockedContent ?? <p>This item is locked</p>}
              </Grid>
            )}
          </Grid>
        </Grid>
        <Grid item className={classes.iconContainer}>
          <Box className={classes.iconBackground}>{iconSelection}</Box>
        </Grid>
      </Grid>

      <Grid item xs={12}>
        <Divider variant="fullWidth" className={classes.headerDivider} />
      </Grid>
    </Grid>
  );
};
