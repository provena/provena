...
import { JobSubType, JobType } from "../provena-interfaces/AsyncJobModels";
    ...
    <Stack direction="row" justifyItems="flex-start" spacing={1.5}>
      ...
      {showingError && (
        <React.Fragment>
          ...
          <p>Error: {wakeUpOp.error ?? "None provided. Check console."}</p>
        </React.Fragment>
      )}
    </Stack>
  );
...
export const JobWakerComponent = (props: JobWakerComponentProps) => {
  ...
  return (
    <Stack className={classes.wakerBox} spacing={2}>
      ...
      <Stack spacing={1.5}>
        {Array.from(JOB_TYPE_WAKE_UP_MAP.entries()).map(([type, subType]) => {
          return (
            <WakeUpComponent
              jobType={type}
              jobSubType={subType}
