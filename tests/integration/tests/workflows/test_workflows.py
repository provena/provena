...
def test_provenance_workflow(
    ...
    model_run_record = ModelRunRecord(
        ...
        inputs=[
            TemplatedDataset(
                ...
                resources={input_deferred_resource_key: "/path/to/resource.csv"},
            )
        ],
        outputs=[
            TemplatedDataset(
                ...
                dataset_type=DatasetType.DATA_STORE,
                resources={output_deferred_resource_key: "/path/to/resource.csv"},
            )
        ],
        associations=AssociationInfo(
            modeller_id=person.id, requesting_organisation_id=organisation.id
        ),
    ...
    activity_upstream_query = successful_basic_prov_query(
        start=model_run_id, direction=Direction.UPSTREAM, depth=1, token=Tokens.user1()
    )

    # model run -wasInformedBy-> study
    assert_non_empty_graph_property(
        prop=GraphProperty(type="wasInformedBy", source=model_run_id, target=study.id),
    ...
    if not failed:
        ...
        assert (
            False
        ), f"Model run registration with invalid study should have failed, but did not."
    ...
    ), f"Model run fetch response was not a model run. Type: {type(model_run)}"
    assert (
        model_run.record.study_id is None
    ), f"Study id should be None but was {model_run.record.study_id}"
    ...
    with pytest.raises(AssertionError):
        assert_non_empty_graph_property(
            prop=GraphProperty(
                type="wasInformedBy", source=model_run_id, target=study.id
            ),
            lineage_response=activity_upstream_query,
        )

    # link to study
    link_resp = link_model_run_to_study_successfully(
        token=write_token, study_id=study.id, model_run_id=model_run_id
    )
    ...
    assert (
        model_run.record.study_id == study.id
    ), f"Study id should be {study.id} but was {model_run.record.study_id}"
    ...
    assert (
        model_run.record.display_name == updated_display_name
    ), f"Model run display name should be {updated_display_name} but was {model_run.record.display_name}"
    assert (
        model_run.record.description == updated_description
    ), f"Model run description should be {updated_description} but was {model_run.record.description}"
...
def test_create_and_update_history(
    dataset_io_fixture: Tuple[str, str],
    linked_person_fixture: ItemPerson,
    organisation_fixture: ItemOrganisation,
) -> None:
    ...
    assert (
        len(history) == 1
    ), f"Should have single item in history but had {len(history)}."
    ...
    put_auth_config(
        id=id,
        auth_payload=py_to_dict(auth_config),
    ...
    assert (
        len(history) == 2
    ), f"Should have two items in history but had {len(history)}."
    ...
    assert (
        len(history) == 3
    ), f"length of history should be three (two versions + update) but was {len(history)}."

    # also check that the new item has the correct original contents
    check_equal_models(
        DatasetDomainInfo.parse_obj(original_item),
        DatasetDomainInfo.parse_obj(py_to_dict(history[0].item)),
    )
    ...
    # create Model
    item = ModelCreateResponse.parse_obj(
        raw_create_item_successfully(
            item_subtype=ItemSubType.MODEL, token=write_token()
        )
    ).created_item
    assert item
    ...
    ), f"Non 200 code: {resp.status_code}, reason: {resp.text}"
    ...
    # also check that the new item has the correct original contents
    check_equal_models(
        domain_info_model.parse_obj(item),
        ModelDomainInfo.parse_obj(py_to_dict(history[0].item)),
    )
