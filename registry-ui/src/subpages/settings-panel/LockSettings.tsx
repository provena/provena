...
import { ItemSubType, LockEvent } from "../../shared-interfaces/RegistryModels";
        ...
        >
            <Grid item>
                <Typography color={color} variant="body1">
                    This resource is{" "}
                    <b>{props.locked ? "locked" : "unlocked"}</b>.
                </Typography>
            </Grid>
            {props.locked && (
                <Grid item>{<LockIcon fontSize={"large"} />}</Grid>
            )}
        </Grid>
    );
});
                                ...
                                <Grid container rowGap={2} xs={12}>
                                    ...
                                    <Grid item xs={12}>
                                        <TextField
                                            ...
                                            label={
                                                "Enter a short justification here"
                                            }
...
const LockHistoryDisplay = observer((props: LockHistoryDisplayProps) => {
    ...
    const headerContent: JSX.Element = (
        <React.Fragment>
            ...
            <Grid item xs={12}>
                Whenever a resource is locked or unlocked, this information is
                recorded.{" "}
                {!hasHistory && (
                    <b>
    ...
    if (showingHistory) {
        ...
        if (props.lockHistory === undefined) {
            bodyContent = (
                <React.Fragment>
                    <Grid item xs={12}>
                        No content is available in the lock history. If this
                        seems wrong, please contact an admin.
                    </Grid>
                </React.Fragment>
            );
        }
    ...
    const headerContent: JSX.Element = (
        <React.Fragment>
            ...
            >
                ...
                <Grid item xs={12}>
                    As an administrator of this resource, you are able to apply
                    or remove a <i>lock</i> which stops users from making any
                    changes to the data or metadata of the resource.{" "}
                    {!expanded && "Expand to see more"}
                </Grid>
            </Grid>
        </React.Fragment>
    );
            ...
            <Grid container rowGap={1.5} alignItems="center">
                ...
                {lockStore.historyAction.ready() ? (
                    <Grid item xs={12}>
                        <LockHistoryDisplay
                            lockHistory={
                                lockStore.historyAction.content?.history
                            }
                        ></LockHistoryDisplay>
                    </Grid>
                ) : (
