...
const subtypeRedirects: Map<ItemSubType, RedirectConfiguration> = new Map([
  ...
  [
    ...
    {
      url: PROV_STORE_LINK,
      description: "To register a Model Run, you can use the Provenance Store.",
    },
  ],
]);
    ...
    <div style={{ width: "100%" }} className={classes.stackContainer}>
      <Grid item>
        ...
        <Typography variant="subtitle1">
          {desc} You can <a href={link}>click here</a> to register an entity of
          this type.
        </Typography>
      </Grid>
    </div>
      ...
      {props.subtype === "PERSON" && props.linked && (
        <div style={{ width: "100%" }}>
          <Alert variant="outlined" severity="warning">
            <AlertTitle>Are you sure you want to register a Person?</AlertTitle>
            Our system has detected that your user account is already linked to
            a Person in the Registry. Are you sure you want to create another
            Person in the Registry? We do not recommend that you register a
            Person on behalf of another user. Please contact us if you are
            uncertain.
          </Alert>
        </div>
      )}
  ...
  if (status?.loading) {
    content = (
      <Stack direction="column" justifyContent="center">
        ...
        >
          <Typography variant="subtitle1">Submitting...please wait</Typography>
          <CircularProgress />
        </Stack>
      </Stack>
    );
      ...
      <Alert
        ...
        action={
          <Stack direction="column" spacing={2}>
            <Button
              onClick={() => {
                let win = window.open(`/item/${props.id}`, "_self");
                win?.focus();
              }}
              variant="outlined"
            >
...
const MissingIdComponent = (props: MissingIdComponentProps) => {
  ...
  return (
    <AlertErrorComponent
      ...
      errorContent={
        <p>
          No ID was provided for the clone or edit operation. No action can be
          taken. Click{" "}
          <b>
            <a href="/">here</a>
          </b>{" "}
...
const RegisterEntity = observer((props: RegisterEntityProps) => {
  ...
  const redirectConfiguration: RedirectConfiguration | undefined =
    selectedItemSubType ? subtypeRedirects.get(selectedItemSubType) : undefined;
  ...
  useEffect(() => {
    if (isCloneMode && relevantSubtype === "PERSON") {
      if (formSetup.existingItem?.data !== undefined) {
        lodashSet(formSetup.existingItem.data, "ethics_approved", false);
      }
    }
  ...
  useEffect(() => {
    if (isCreateMode) {
      ...
      if (!!relevantSubtype) {
        ...
        if (possibleAutofill !== undefined && !!kc.keycloak.tokenParsed) {
          possibleAutofill({
  ...
  const selector = (
    <div className={classes.stackContainer}>
      <Stack spacing={2}>
        <Typography variant="h4">
          Select the type of item you would like to register
        </Typography>
        <ItemSubtypeSelector
          selectedItemSubType={selectedItemSubType ?? defaultSelectedSubtype}
  ...
  const overridedMissingLinkMessage = (
    <p>
      ...
      {isCreateMode
        ...
        ? "clone"
        : "create"}{" "}
      an entity without linking your user account to a Person in the Registry.
      If you are trying to register a Person, use the selection tool below to
      select "Person". For more information, visit{" "}
      <a
  ...
  // Should we show the special person link banner?
  const showPersonLinkId: string | undefined =
    !linkEnforcer.validLink &&
    relevantSubtype &&
    relevantSubtype === "PERSON" &&
    createManager.response.success
      ? createManager.response.data?.created_item?.id
      : undefined;
  ...
  return (
    <Stack spacing={2} direction="column">
      ...
      {linkEnforcer.blocked && (
        <div className={classes.stackContainer}>{linkEnforcer.render}</div>
      )}
      {
        // Access loading/issue
      }
      {!auth.granted && <AccessStatusDisplayComponent accessCheck={auth} />}
      {
      ...
      {
        // If Person, not already linked, successfully created, show the link person
      }
      {showPersonLinkId && (
        <div className={classes.stackContainer}>
          <LinkPersonBoxComponent
            person_id={showPersonLinkId}
          ></LinkPersonBoxComponent>
        </div>
      )}
      {
